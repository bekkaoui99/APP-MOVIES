...
$total_items_query = $connection->prepare("SELECT COUNT(*) as total FROM Films where projection=0");
...
$query = $connection->prepare("SELECT * FROM Films where projection=0 LIMIT $items_per_page OFFSET $offset");
$query->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['Search'])) {
        if (ctype_alpha($_POST['Search_category'])) {
            $query = $connection->prepare("SELECT * FROM Films  WHERE name LIKE :name  and  projection=0 LIMIT $items_per_page OFFSET $offset");
            ...
            $query = $connection->prepare("SELECT * FROM Films  WHERE anne = :anne  and  projection=0 LIMIT $items_per_page OFFSET $offset");
